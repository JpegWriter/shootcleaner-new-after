...
function createWindow() {
    ...
    mainWindow = new electron_1.BrowserWindow({
        ...
        webPreferences: {
            preload: (0, path_1.join)(__dirname, is.dev ? '../preload.js' : './preload.js'),
            sandbox: false,
...
electron_1.ipcMain.handle('fs:ensureDirectories', async (_event, dirs) => {
    ...
    electron_1.ipcMain.handle('imagemagick:batchProcess', async (_, { images, settings }) => {
        ...
        for (const imageData of images) {
            try {
                ...
                await fs_1.promises.writeFile(inputPath, Buffer.from(imageData.buffer));
